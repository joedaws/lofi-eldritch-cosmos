defmodule Cosmos.EntityCacheTest do
  ...
  test "server process for new being" do
    Cosmos.EntityCache.start_link()

    server = Cosmos.EntityCache.server_process()
    entity_id = Cosmos.EntityServer.get(server).id

    assert server == Cosmos.EntityCache.server_process(entity_id)
  end
  ...
  test "server process for existing being" do
    ...
    new_server = Cosmos.EntityCache.server_process(entity_created.id)
